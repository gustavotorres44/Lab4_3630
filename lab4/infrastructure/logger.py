...
class Logger:
    def __init__(self, log_dir, n_logged_samples=10, summary_writer=None):
        ...
        # CSV logging for easier visualization
    ...
    def log_scalar(self, scalar, name, step_):
        ...
        # Also log to CSV
        if step_ != self._csv_step:
            # New step - write previous row if exists
            if self._csv_data and self._csv_step >= 0:
                self._write_csv_row()
            self._csv_step = step_
            self._csv_data = {'step': step_}
        self._csv_data[name] = scalar
    ...
    def _write_csv_row(self):
        """Write the current row to CSV file"""
        ...
        with open(self._csv_path, 'a', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=sorted(self._csv_data.keys()))
            if not file_exists:
                writer.writeheader()
